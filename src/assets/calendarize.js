...
//console.log("This from calendar");
		...
		buildYearCalendar: function(el, year) {
			...
			var opts = {
				...
				clickHandler: function(e) {
					...
					if(Date.parse(day) < new Date()){ // La fecha es pasada, no se puede editar
						//return;
						$('#selectClaseDia')[0].disabled = true;
						$('#selectTipoDia')[0].disabled = true;
					}else{ // La fecha si se puede editar
						$('#selectClaseDia')[0].disabled = false;
						$('#selectTipoDia')[0].disabled = false;
					}

					// console.log(day)
					// console.log(new Date())

					_datacalendar = JSON.parse(id);
					$('#lblFecha').text(new Date(day).toLocaleDateString());

					$('#selectClaseDia').val(_datacalendar.claseDia);
					$('#selectTipoDia').val(_datacalendar.tipoDia);
					$('#EditDateModal').modal();
				}
			};
